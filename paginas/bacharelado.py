...
df_bacharelado = historico[(historico.tipo == 'Bacharelado')&(~historico.mencao.isin(["CC", "DP"]))]
df_bacharelado['mencao'].replace({"TJ":"TR"}, inplace=True)

def app():
    st.title("Disciplinas do Bacharelado")
    st.write("Informações das disciplinas do bacharelado de Estatística")

    #st.write(df_bacharelado.mencao.value_counts())

    disciplinas_ordenadas = sorted(df_bacharelado.disciplina.unique())
    ...
    if len(disc_selecionadas) > 0:
        disciplinas = df_bacharelado[df_bacharelado.disciplina.isin(disc_selecionadas)]
    else:
        disciplinas = df_bacharelado
    ...
    #aprovacao = mencoes_disciplinas[mencoes_disciplinas.mencoes.isin(["SS", "MS", "MM"])].groupby('disciplina').mencao.count().sort_values(ascending=True)
    #aprovacao = mencoes_disciplinas[mencoes_disciplinas.mencoes.isin(["SS", "MS", "MM"])].groupby('disciplina').mencao.sum().sort_values(ascending=False).index
        ...
        fig_linhas = px.line(mencoes_periodos, x = 'periodo', y='mencao', color='mencoes', labels={'periodo': 'Período', 'mencao': 'Quantidade de Alunos', 'mencoes': 'Menção'},
                    category_orders={"mencoes": ["SS", "MS", "MM", "TR", "MI", "II", "SR"],
                                     "periodo": ["1994/1","1994/2","1995/1","1995/2","1996/1","1996/2","1997/1","1997/2","1998/1","1998/2","1999/1","1999/2",
                                                 ...
                                                 "2010/1","2010/2","2011/1","2011/2","2012/0","2012/1","2012/2","2013/1","2013/2","2014/0","2014/1","2014/2","2015/1","2015/2","2016/1","2016/2","2017/1","2017/2","2018/1","2018/2","2019/0","2019/1","2019/2",
                                                 "2020/0"]}, 
