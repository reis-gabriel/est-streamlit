...
df_servico = historico[(historico.tipo == 'Matemática')&(~historico.mencao.isin(["CC", "DP"]))]
df_servico['mencao'].replace({"TJ":"TR"}, inplace=True)

def app():
    st.title("Disciplinas da Matemática")
    st.write("Informações das disciplinas do Departamento da Matemática")
    ...
    if absoluto:
        fig_linhas = px.line(mencoes_periodos, x = 'periodo', y='mencao', color='mencoes', labels={'periodo': 'Período', 'mencao': 'Quantidade de Alunos', 'mencoes': 'Menção'},
                    category_orders={"mencoes": ["SS", "MS", "MM", "TR", "MI", "II", "SR"],
                                     "periodo": ["1994/1","1994/2","1995/1","1995/2","1996/1","1996/2","1997/1","1997/2","1998/0","1998/1","1998/2","1999/1","1999/2",
